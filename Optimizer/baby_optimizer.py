...
class Pusher():
    def __init__(self, order, num_pieces):
        self.order_number = order.order_number
        self.piece_type = order.piece_type
...
class Optimizer:
    ...
    def order_handler(self, order):
        ...
        #
            ...
            dest_path = {48: [3, 8, 15, 20, 27, 32, 39, 41, 42, 48], 49: [3, 8, 15, 20, 27, 32, 39, 41, 42, 43, 49], 50: [3, 8, 15, 20, 27, 32, 39, 41, 42, 43, 44, 50]}
            for piece_number in range(self.state.num_pieces, self.state.num_pieces + 4):
                self.state.pieces[piece_number] = \
            ...
            #append numa fila pusher order.quantity-self.state.num_pieces
            if(self.state.num_pieces < order.quantity):
                self.pusher= Pusher(order, self.state.num_pieces )
                self.pusher.dispatch_queue.append(self.state.pieces[piece_number])
